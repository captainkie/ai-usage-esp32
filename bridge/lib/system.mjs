...
export function parseDf(dfOut) {
  ...
  if (rows.length < 2) return null;                        // need header + a data row
  const line = rows.at(-1).trim().split(/\s+/);
...
export function parseTop(psOut, n = 3) {
  return psOut.split("\n").map((l) => l.trim()).filter(Boolean)
    ...
    .map((m) => ({ cpu: Math.round(parseFloat(m[1])), name: m[2].split("/").at(-1).trim() }))
    .slice(0, n);
}
...
export async function readSystem(prevNet = null) {
  ...
  return {
    system: {
      ...
      disk: parseDf(df),
      // real elapsed time between samples, not a fixed assumption
      net: prevNet && prevNet.at ? netRate(prevNet, curNet, now - prevNet.at) : { down_kbps: 0, up_kbps: 0 },
